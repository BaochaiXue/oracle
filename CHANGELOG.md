...
- Integrate upstream through `a10ef152`: trusted dispatch revalidation,
  exactly-once input-event handling, owned attachment cleanup, exact-target
  recovery, and locale-independent structural Pro-effort verification.
- Preserve Latest/GPT-6 Pro for every new turn, the sole verified pre-Send
  Sol Pro fallback, Markdown/virtualized prompt identity, two-hour capture,
  WSL process identity, and shared-browser ownership protections.
- Never classify a post-dispatch timeout as retry-safe based on prompt length.
  Truncated pre-Send drafts remain owned and retained until cleanup is proved.
- Import v2 durable-job and sandbox source as explicit opt-in only. Defer the
  macOS-only Batch cutover; this fork keeps direct-CDP Batch and existing
  recovery on supported platforms. See `docs/upstream-integration.md`.
- Include the imported browser runtime's Playwright dependency in production
  installations, and verify broker/worker modules load from the packed CLI.
- Preserve POSIX profile paths for Linux sandbox Chrome launched under WSL;
  do not let chrome-launcher create a relative UNC profile in the checkout.
  macOS and Windows executable handling remain unchanged.
...
- Oracle v2 opt-in broker candidate: add a durable job kernel, SQLite/CAS
  ...
  recovery. Upstream Batch-owned durable execution remains deferred in this fork. The legacy

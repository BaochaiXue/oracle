...
describe("Oracle v2 certified browser runtime", () => {
  test("preserves a Linux sandbox profile path without WSL UNC conversion", () => {
    const options = managedBrowserTestHooks.buildManagedChromeLaunchOptions(
      {
        executablePath: "/opt/chrome/chrome",
        profileDir: "/tmp/oracle-example/profile",
        headless: true,
      },
      "linux",
    );
    expect(options.userDataDir).toBe(false);
    expect(options.chromeFlags).toContain("--user-data-dir=/tmp/oracle-example/profile");
  });

  test.each(["darwin", "win32"] as const)(
    "preserves the %s launcher profile contract",
    (platform) => {
      const options = managedBrowserTestHooks.buildManagedChromeLaunchOptions(
        {
          executablePath: "/opt/chrome/chrome",
      ...
      expect(options.userDataDir).toBe("/tmp/oracle-example/profile");
      expect(options.chromeFlags.some((flag) => flag.startsWith("--user-data-dir="))).toBe(false);
    },
  );

  test("leaves a WSL Windows executable to chrome-launcher's path bridge", () => {
    const options = managedBrowserTestHooks.buildManagedChromeLaunchOptions(
      {
        executablePath: "/mnt/c/chrome.exe",
        profileDir: "/tmp/oracle-example/profile",
        headless: false,
      },
      "linux",

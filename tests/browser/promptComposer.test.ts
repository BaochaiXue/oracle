...
describe("promptComposer", () => {
  ...
  test.each([false, true])(
    "never infers a large prompt was unsent (new user turn: %s)",
    async (hasNewUserTurn) => {
      vi.useFakeTimers();
      try {
        const runtime = {
          evaluate: vi.fn(async () => ({
            result: {
              value: {
                baseline: 0,
                turnsCount: hasNewUserTurn ? 1 : 0,
                userMatched: false,
                hasNewTurn: hasNewUserTurn,
                hasNewUserTurn,
                stopVisible: false,
                assistantVisible: false,
                composerCleared: true,
        ...
        const assertion = expect(promise).rejects.toMatchObject({
          details: expect.objectContaining({
            ...
            retrySafe: false,
            ...(hasNewUserTurn
              ? { promptSubmitted: true, submissionState: "submitted-unverified" }
              : {}),
          }),
        });
        await vi.advanceTimersByTimeAsync(250);
  ...
  test.each([5, 50_000])(
    "keeps a dispatched %i-character attachment prompt indeterminate",
    async (length) => {
        ...
        const assertion = expect(result).rejects.toMatchObject({
          details: expect.objectContaining({
            ...
            submissionDiagnostic: expect.objectContaining({
              ...
              composerLengthBeforeDispatch: prompt.length,
              composerCleared: false,
              draftRetained: true,

...
| `apps/oracle-worker/src/certifiedProvider.ts`                            | Singleton runtime/adapter and shared recovery target preservation | Router keyed by `jobId + turnAttemptId + purpose`; dispatch, at-risk, capture, and probe sandboxes have explicit lifetimes                                |
...
- T2: integrate the sandbox router and job-local cleanup under fixture/fault
  evidence; no unapproved live Send.

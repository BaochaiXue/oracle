...
- Archiving: conversations are preserved by default (`browserArchive:"never"`) for inspection and manual follow-up. Set `browserArchive:"auto"` to archive only successful browser one-shots after local artifacts are saved while skipping project, Deep Research, multi-turn, failed, and incomplete sessions; use `"always"` for explicit archive-all-on-success behavior.

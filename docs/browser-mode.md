...
- Each submitted Pro turn stores its own dispatch timestamp, estimated input tokens, uploaded bytes, normalized prompt digest, and exact committed user-turn index. A substantive turn (more than 256 estimated input tokens or 16 KiB of uploads) captured in under 60 seconds is rejected instead of being presented as trusted Pro output. Tiny prompts may legitimately return faster. Reattach verifies the stored prompt digest at the stored turn index before accepting an answer, so a follow-up cannot borrow the initial turn's workload or recover from a neighboring conversation turn. Sessions created before these markers existed remain recoverable; once any timing marker is present, invalid or incomplete timing evidence fails closed.

...
- `--browser-headless`, `--browser-hide-window`, `--browser-keep-browser`, and the global `-v/--verbose` flag control the launcher and diagnostics. On macOS, a later visible run restores a locally launched Oracle window that was previously positioned off-screen; attach-running and remote Chrome windows are never repositioned by this policy.

...
not redispatch or wait for a response whose existence is unknown. Reattach
requires the stored exact target ID and never falls back to another tab for the
same conversation or a newly launched browser. The durable
dispatch-boundary marker must be written before either event; persistence
failure prevents dispatch. After that write returns, Oracle revalidates exact
target ownership, prompt identity, attachment readiness, and the current
hit-tested Send point before emitting the input. A retained pre-dispatch draft or other
...
2. **Automation stack** – code lives under `src/browser/`:
   ...
   - Immediately before dispatch, Oracle activates the exact owned target, rechecks ownership and the visible composer, and measures a fresh trusted Send point. Coordinate clicks and Enter share the same exact-turn verification. The durable dispatch-boundary marker must be persisted before either potentially submitting input event. For trusted-click, Oracle moves the pointer first, revalidates the exact target again, and then repeats the complete-prompt, exact-attachment-set, and hit-test probes immediately before `mousePressed`. Enter is permitted as an alternate only when trusted-click emitted no potentially submitting input event at all. Once `mousePressed` or Enter `keyDown` has been emitted, Oracle never changes methods or dispatches again automatically.
   - Oracle refuses to overwrite or append to non-empty composer content it cannot prove it owns. After a potentially submitting event, retained drafts and cleared-but-unobserved commits are classified as indeterminate/recoverable with `retrySafe:false` and keep their exact tab with sanitized submission diagnostics. Any persisted current-prompt digest without a verified committed turn index is exact-target-only even when interruption occurred before a non-Pro run wrote `recoveryKind`. Preserve that session and inspect it with `oracle session <id> --render`; do not rerun the prompt while commit state is uncertain.
   - A first non-empty composer observation receives up to five seconds of read-only settling checks for transient profile/SPA restoration; persistent content remains untouched. Before each run begins any upload, typing, or dispatch, Oracle proves that composer text and the attachment set are both empty, including for text-only submissions, and never sweeps pre-existing attachment UI; attachment-bearing runs later require their exact owned set before dispatch. A temporarily missing Send control while an attachment is processing remains inside the bounded attachment wait rather than causing an immediate Enter fallback or another dispatch. Before any submitting event, an attachment Send-readiness failure may clear only the current attempt's attachments and exact prompt after exact-target, ownership, current full-prompt, and exact attachment-set re-verification. Attachment removal atomically revalidates that exact set and clicks only the controls captured in that snapshot; if ChatGPT retains a hidden input `FileList`, Oracle resets it only when that complete input set independently matches the attempt. A final single DOM snapshot must then prove that both the composer text and current attachment set are empty. The automatic inline-to-file fallback passes the first attempt's prompt and attachment identity into the same exact-owned discipline; a `prompt-too-large` failure binds cleanup to the normalized observed truncated-draft digest instead of the unavailable full prompt. Oracle removes that owned state only after re-verification, then uploads the fallback set. Verified cleanup records `retrySafe:true`; unverifiable or partial cleanup retains the exact tab with `retrySafe:false`. A large prompt truncated before dispatch without a usable file fallback is also a retained draft and keeps its exact tab with `retrySafe:false`. Failures that occur before any submitting event and before any draft is retained are explicitly retry-safe instead of advertising a nonexistent reattach path.
...
### GPT-6 Pro default and fallback

New browser runs default to GPT-6 Pro via `gpt-5-pro` (also `gpt-6-pro`).
The picker may label the row `Latest`; Oracle requires version 6 in the
...
- `--browser-timeout`, `--browser-input-timeout`, `--browser-attachment-timeout`: the ordinary per-attempt defaults are `1200s (20m)`/`60s`/`180s (3m)`. Pro uses a `3600s (60m)` response-capture attempt and may reload the same bound conversation once without resubmitting, for a two-hour default capture ceiling. Deep Research keeps its separate `2400s (40m)` default. The attachment timeout controls upload/readiness before clicking Send and can also be set with `ORACLE_BROWSER_ATTACHMENT_TIMEOUT` or `browser.attachmentTimeoutMs`. Multi-file text uploads are automatically consolidated into one text bundle; media remains a separate native upload unless explicit ZIP bundling is requested. Durations accept `ms`, `s`, `m`, or `h` and can be chained (`1h2m10s`).
...
- If an assistant response still times out (common with long Pro runs), Oracle marks the session as an incomplete capture, stores reattach/runtime diagnostics, and keeps enough browser metadata for `oracle session <id>` to recover the final answer. `oracle session <id> --live` treats an active Pro stop control as positive generation liveness and keeps waiting up to the same two-hour default ceiling instead of declaring a one-minute text stall. Visible ChatGPT rate-limit, temporary-unavailable, and authentication/challenge warnings are included in the error and session metadata instead of being reduced to a generic timeout. Increase `--browser-timeout` only when the browser session is truly unrecoverable.

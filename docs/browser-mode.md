...
2. **Automation stack** – code lives under `src/browser/`:
   ...
   - A first non-empty composer observation receives up to five seconds of read-only settling checks for transient profile/SPA restoration; persistent content remains untouched. Before each run begins any upload, typing, or dispatch, Oracle proves that composer text and the attachment set are both empty, including for text-only submissions, and never sweeps pre-existing attachment UI; attachment-bearing runs later require their exact owned set before dispatch. Before any submitting event, an attachment Send-readiness failure may clear only the current attempt's attachments and exact prompt after exact-target, ownership, current full-prompt, and exact attachment-set re-verification. Attachment removal atomically revalidates that exact set and clicks only the controls captured in that snapshot; a final single DOM snapshot must then prove that both the composer text and current attachment set are empty. The automatic inline-to-file fallback passes the first attempt's attachment identity into the same exact-set discipline, removes that owned set only after re-verification, and then uploads the fallback set. Verified cleanup records `retrySafe:true`; unverifiable or partial cleanup retains the exact tab with `retrySafe:false`. A large prompt truncated before dispatch without a usable file fallback is also a retained draft and keeps its exact tab with `retrySafe:false`. Failures that occur before any submitting event and before any draft is retained are explicitly retry-safe instead of advertising a nonexistent reattach path.

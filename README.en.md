...
Legacy direct-CDP submission activates and revalidates the exact owned target before measuring a fresh trusted Send point; clicks and Enter share one exact-user-turn verification path. Every potentially submitting input event requires successful durable dispatch-boundary persistence first; a persistence failure prevents the event. Oracle may move from an unavailable trusted-click path to Enter only before any submitting event has been emitted. After `mousePressed` or Enter `keyDown`, it never changes methods or dispatches again automatically. If the exact commit cannot be verified, an ordinary headful dedicated-profile run is persisted as incomplete/recoverable with `retrySafe:false` and preserves the exact tab for `oracle session <session-id> --render` reattach. Because `--copy-profile` always deletes its temporary profile and `--browser-headless` does not retain its browser process, the same ambiguity is explicitly non-reattachable in those modes and still blocks automatic rerun.
